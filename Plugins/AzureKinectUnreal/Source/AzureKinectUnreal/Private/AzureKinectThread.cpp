...
FAzureKinectThread* FAzureKinectThread::Instance2 = nullptr;
	...
	KinectDevice(Device)
{
	...
	if (!KinectThread)
	{
		...
		UE_LOG(AzureKinectThreadLog, Warning, TEXT("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"));
}
...
FAzureKinectThread * FAzureKinectThread::InitPolling(AzureKinectDevice *Device)
{
	...
	else if (!Instance2 && FPlatformProcess::SupportsMultithreading())
	{
		Instance2 = new FAzureKinectThread(Device);
		return Instance2;
	}
