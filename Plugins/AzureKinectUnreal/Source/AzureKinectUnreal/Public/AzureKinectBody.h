...
struct FAzureKinectJoint
{
	...
	{
		confidence = (int32)NativeJoint.confidence_level;
		customIndex = (EKinectBodyJointId)Index;
...
/*#define JOINT_ID(Name) static_cast<uint8>(EKinectBodyJointId::Name)
		...
		else if (Index == JOINT_ID(ELBOW_LEFT) || Index == JOINT_ID(ELBOW_RIGHT))
			//|| Index == JOINT_ID(WRIST_LEFT) || Index == JOINT_ID(WRIST_RIGHT))
		{
			Orientation = FRotationMatrix::MakeFromXZ(JointQuaternion.GetAxisY(), -JointQuaternion.GetAxisZ())
				.Rotator();
		}
		else if (Index == JOINT_ID(WRIST_LEFT) || Index == JOINT_ID(WRIST_RIGHT))
		{
			Orientation = FRotationMatrix::MakeFromXZ(JointQuaternion.GetAxisY(), -JointQuaternion.GetAxisZ())
				.Rotator();
		}
